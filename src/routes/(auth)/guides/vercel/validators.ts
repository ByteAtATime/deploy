export async function validateGithubReadmeComment(
	repoUrl: string
...
export async function validateOwnerCommit(
	repoUrl: string
): Promise<{ success: boolean; message: string }> {
	try {
		const urlPattern = /github\.com\/([^\/]+)\/([^\/]+)/;
...
export function createVercelDeploymentValidator(slackId: string) {
	return async function validateVercelDeployment(
		deployUrl: string
	): Promise<{ success: boolean; message: string }> {
		try {
...
export async function validateVercelDeployment(
	deployUrl: string
): Promise<{ success: boolean; message: string }> {
	try {
		if (!deployUrl.includes('vercel.app')) {
